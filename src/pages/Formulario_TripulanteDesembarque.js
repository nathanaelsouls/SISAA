...
export default class Formulario_TripulanteDesembarque extends React.Component{
  constructor(props) {
      ...
      this.state = {
        ...
        //Dados
        Voo: "", EmpresaSolicitante: "", Prefixo:         "", NomeSolicitante: "",
                  QuantidadeViagens:  "", HoraSolicitacao: "", Posicao:         ""
      };
  }
          ...
          <ScrollView style={styles.container}>
              <View style={styles.container}>
                  <Card style={styles.containercard}>
                      ...
                      />
                      <Text style={styles.text}>Quantidade de viagens:*</Text>
                      <TextInput
                      style={styles.inputBox}
                      onChangeText={(text) => this.setState({QuantidadeViagens: text})}
                      placeholder="Quantidade de viagens"
                      value={this.state.QuantidadeViagens} 
                      />
  ...
  askRegister(){    
    ...
    if(validar){    
        Alert.alert(
          'Registrar',
          'Confirma Formulário Tripulante e/ou Desembarque?\nEmpresa: ' + this.state.EmpresaSolicitante ,
          [
            {text: 'Cancelar', onPress: () => console.log('Cancel Pressed'), style: 'cancel'},
            {text: 'OK', onPress: () =>
              this.confirmRegister(this.state.userData.matricula,  this.state.userData.nome,      this.state.date,
                this.state.userData.Voo,    this.state.EmpresaSolicitante, this.state.Prefixo,    this.state.NomeSolicitante,
                this.state.HoraSolicitacao, this.state.QuantidadeViagens,  this.state.Posicao
              )},
          ],
          { cancelable: false }
        )
    ...
    confirmRegister () {
      const userData = {
        ...
        _09_Quantidade_Viagens:             this.state.QuantidadeViagens,
        _10_Posição:                        this.state.Posicao    
      }
        firebase.database().ref("DAPE_TripulanteDesembarque/").push(userData)
        .then((snapshot) => {
    ...
    ValidarCampos(){    
      ...
      if(this.state.QuantidadeViagens == null || this.state.QuantidadeViagens == "") {
        Alert.alert('Atenção!', 'Informe a quantidade de viagens.');
        return false;
      }
